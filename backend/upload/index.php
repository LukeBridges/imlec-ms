...
$default_timezone = 'Etc/GMT+0'; // UTC
...
defined('FM_ROOT_URL') || define('FM_ROOT_URL',
    ($is_https ? 'https' : 'http') . '://' . $http_host . (!empty($root_url) ? '/' . $root_url : ''));
defined('FM_SELF_URL') || define('FM_SELF_URL',
    ($is_https ? 'https' : 'http') . '://' . $http_host . $_SERVER['PHP_SELF']);
    ...
    if (isset($_SESSION['logged'], $auth_users[$_SESSION['logged']])) {
        ...
        <div class="path login-form">
            <form action="" method="post">
                <label for="fm_usr"><?php echo fm_t('Username', $lang) ?></label><input type="text" id="fm_usr"
                                                                                        name="fm_usr" value=""
                                                                                        placeholder="<?php echo fm_t('Username',
                                                                                            $lang) ?>" required><br>
                <label for="fm_pwd"><?php echo fm_t('Password', $lang) ?></label><input type="password" id="fm_pwd"
                                                                                        name="fm_pwd" value=""
                                                                                        placeholder="<?php echo fm_t('Password',
                                                                                            $lang) ?>" required><br>
...
define('FM_READONLY',
    $use_auth && !empty($readonly_users) && isset($_SESSION['logged']) && in_array($_SESSION['logged'],
        $readonly_users));
...
if (isset($_GET['upload']) && !FM_READONLY) {
    ...
    <div class="path">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <br>
            <p>
                <button type="submit" class="btn"><i class="fa fa-check-circle"></i> <?php echo fm_t('Upload', $lang) ?>
                </button> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i
                                class="fa fa-times-circle"></i> <?php echo fm_t('Cancel', $lang) ?></a></b>
            </p>
        </form>
    </div>
    <?php
...
if (isset($_POST['copy']) && !FM_READONLY) {
    ...
    <div class="path">
        ...
        <form action="" method="post">
            ...
            ?>
            <p class="break-word"><?php echo fm_t('Files:') ?> <b><?php echo implode('</b>, <b>', $copy_files) ?></b>
            </p>
            <p class="break-word"><?php echo fm_t('Source folder:') ?> <?php echo fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH) ?>
                <br>
            ...
            <p>
                <button type="submit" class="btn"><i class="fa fa-check-circle"></i> <?php echo fm_t('Copy', $lang) ?>
                </button> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i
                                class="fa fa-times-circle"></i> <?php echo fm_t('Cancel', $lang) ?></a></b>
            </p>
    ...
    <div class="path">
        ...
        <p>
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode($copy) ?>&amp;finish=1"><i
                            class="fa fa-check-circle"></i> <?php echo fm_t('Copy', $lang) ?></a></b> &nbsp;
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode($copy) ?>&amp;finish=1&amp;move=1"><i
                            class="fa fa-check-circle"></i> <?php echo fm_t('Move', $lang) ?></a></b> &nbsp;
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i
                            class="fa fa-times-circle"></i> <?php echo fm_t('Cancel', $lang) ?></a></b>
        </p>
...
if (isset($_GET['view'])) {
    ...
    if ($ext == 'zip') {
        ...
        $is_video = true;
        $view_title = 'Video';
    } elseif (in_array($ext, fm_get_text_exts()) || substr($mime_type, 0, 4) == 'text' || in_array($mime_type,
            fm_get_text_mimes())) {
        $is_text = true;
    ...
    <div class="path">
        ...
        <p class="break-word">
            ...
            <?php echo fm_t('File size:',
                $lang) ?> <?php echo fm_get_filesize($filesize) ?><?php if ($filesize >= 1000): ?> (<?php echo sprintf('%s bytes',
                $filesize) ?>)<?php endif; ?><br>
        ...
        <p>
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($file) ?>"><i
                            class="fa fa-cloud-download"></i> <?php echo fm_t('Download', $lang) ?></a></b> &nbsp;
            <b><a href="<?php echo $file_url ?>" target="_blank"><i
                            class="fa fa-external-link-square"></i> <?php echo fm_t('Open', $lang) ?></a></b> &nbsp;
            <?php
            // ZIP actions
            if (!FM_READONLY && $is_zip && $filenames !== false) {
                ...
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;unzip=<?php echo urlencode($file) ?>"><i
                                class="fa fa-check-circle"></i> <?php echo fm_t('UnZip', $lang) ?></a></b> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;unzip=<?php echo urlencode($file) ?>&amp;tofolder=1"
                      title="<?php echo fm_t('UnZip to', $lang) ?> <?php echo fm_enc($zip_name) ?>"><i
            ...
            if ($is_text && !FM_READONLY) {
                ...
                <b><a href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>"
                      class="edit-file"><i class="fa fa-pencil-square"></i> <?php echo fm_t('Edit', $lang) ?>
                    </a></b> &nbsp;
                <b><a href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>&env=ace"
                      class="edit-file"><i class="fa fa-pencil-square"></i> <?php echo fm_t('Advanced Edit', $lang) ?>
                    </a></b> &nbsp;
            <?php }
            if ($send_mail && !FM_READONLY) {
                ?>
                <b><a href="javascript:mailto('<?php echo urlencode(trim(FM_ROOT_PATH . '/' . FM_PATH)) ?>','<?php echo urlencode($file) ?>')"><i
                                class="fa fa-pencil-square"></i> <?php echo fm_t('Mail', $lang) ?></a></b> &nbsp;
            <?php } ?>
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i
                            class="fa fa-chevron-circle-left"></i> <?php echo fm_t('Back', $lang) ?></a></b>
        </p>
        <?php
        if ($is_zip) {
            // ZIP content
            ...
            if (FM_USE_HIGHLIGHTJS) {
                ...
                if (empty($ext) || in_array(strtolower($file), fm_get_text_names()) || preg_match('#\.min\.(css|js)$#i',
                        $file)) {
...
if (isset($_GET['edit'])) {
    ...
    if (in_array($ext, fm_get_text_exts()) || substr($mime_type, 0, 4) == 'text' || in_array($mime_type,
            fm_get_text_mimes())) {
        $is_text = true;
        $content = file_get_contents($file_path);
    }
    ...
    <div class="path">
        <div class="edit-file-actions">
            <a title="<?php echo fm_t('Cancel', $lang) ?>"
               href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;view=<?php echo urlencode($file) ?>"><i
                        class="fa fa-reply-all"></i> <?php echo fm_t('Cancel', $lang) ?></a>
            <a title="<?php echo fm_t('Backup', $lang) ?>"
               href="javascript:backup('<?php echo urlencode($path) ?>','<?php echo urlencode($file) ?>')"><i
                        class="fa fa-database"></i> <?php echo fm_t('Backup', $lang) ?></a>
            <?php if ($is_text) { ?>
                <?php if ($isNormalEditor) { ?>
                    <a title="<?php echo fm_t('Advanced', $lang) ?>"
                       href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>&amp;env=ace"><i
                                class="fa fa-paper-plane"></i> <?php echo fm_t('Advanced Editor', $lang) ?></a>
                    <button type="button" name="<?php echo fm_t('Save', $lang) ?>" data-url="<?php echo $file_url ?>"
                            onclick="edit_save(this,'nrl')"><i class="fa fa-floppy-o"></i> <?php echo fm_t('Save',
                            $lang) ?></button>
                <?php } else { ?>
                    <a title="<?php echo fm_t('Plain Editor', $lang) ?>"
                       href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>"><i
                                class="fa fa-text-height"></i> <?php echo fm_t('Plain Editor', $lang) ?></a>
                    <button type="button" name="Save" data-url="<?php echo $file_url ?>"
                            onclick="edit_save(this,'ace')"><i class="fa fa-floppy-o"></i> <?php echo fm_t('Save',
                            $lang) ?></button>
                <?php } ?>
            <?php } ?>
        </div>
...
if (isset($_GET['chmod']) && !FM_READONLY && !FM_IS_WIN) {
    ...
    <div class="path">
        ...
        <form action="" method="post">
            ...
            <p>
                <button type="submit" class="btn"><i class="fa fa-check-circle"></i> <?php echo fm_t('Change', $lang) ?>
                </button> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i
                                class="fa fa-times-circle"></i> <?php echo fm_t('Cancel', $lang) ?></a></b>
            </p>
...
?>
    <form action="" method="post">
        ...
        <table class="table">
            ...
            <tr>
                <?php if (!FM_READONLY): ?>
                    <th style="width:3%"><label><input type="checkbox"
                                                       title="<?php echo fm_t('Invert selection', $lang) ?>"
                                                       onclick="checkbox_toggle()"></label></th><?php endif; ?>
                <th><?php echo fm_t('Name', $lang) ?></th>
                <th style="width:10%"><?php echo fm_t('Size', $lang) ?></th>
                <th style="width:12%"><?php echo fm_t('Modified', $lang) ?></th>
                <?php if (!FM_IS_WIN): ?>
                    <th style="width:6%"><?php echo fm_t('Perms', $lang) ?></th>
                    <th style="width:10%"><?php echo fm_t('Owner', $lang) ?></th><?php endif; ?>
                <th style="width:<?php if (!FM_READONLY): ?>13<?php else: ?>6.5<?php endif; ?>%"><?php echo fm_t('Actions',
                        $lang) ?></th>
            </tr>
            </thead>
            <?php
            ...
            foreach ($folders as $f) {
                ...
                <tr>
                    ...
                    <td><?php echo fm_t('Folder', $lang) ?></td>
                    <td><?php echo $modif ?></td>
                    <?php if (!FM_IS_WIN): ?>
                        <td><?php if (!FM_READONLY): ?><a title="<?php echo fm_t('Change Permissions', $lang) ?>"
                    ...
                    <td class="inline-actions"><?php if (!FM_READONLY): ?>
                            <a title="<?php echo fm_t('Delete', $lang) ?>"
                               href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>"
                               onclick="return confirm('<?php echo fm_t('Delete folder?') ?>');"><i
                                        class="fa fa-trash-o" style="background:#FF4136" aria-hidden="true"></i></a>
                            <a title="<?php echo fm_t('Rename', $lang) ?>" href="#"
                               onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;"><i
                                        class="fa fa-pencil-square-o" style="background:#FF851B" aria-hidden="true"></i></a>
                            <a title="<?php echo fm_t('Copy to...') ?>"
                               href="?p=&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i
                                        class="fa fa-files-o" aria-hidden="true"></i></a>
                        <?php endif; ?>
                        <a title="<?php echo fm_t('Direct link', $lang) ?>"
                           href="<?php echo FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f . '/' ?>"
                           target="_blank"><i class="fa fa-link" style="background:#001f3f" aria-hidden="true"></i></a>
                    </td>
                </tr>
                <?php
                flush();
                ...
                <tr>
                    ...
                    <td>
                        <div class="filename"><a href="<?php echo $filelink ?>"
                                                 title="<?php echo fm_t('File info', $lang) ?>"><i
                                        class="<?php echo $img ?>"></i> <?php echo fm_convert_win($f) ?>
                            </a><?php echo($is_link ? ' &rarr; <i>' . readlink($path . '/' . $f) . '</i>' : '') ?></div>
                    </td>
                    <td><span title="<?php printf(fm_t('%s bytes'), $filesize_raw) ?>"><?php echo $filesize ?></span>
                    </td>
                    <td><?php echo $modif ?></td>
                    <?php if (!FM_IS_WIN): ?>
                        <td><?php if (!FM_READONLY): ?><a title="<?php echo fm_t('Change Permissions', $lang) ?>"
                    ...
                    <td class="inline-actions">
                        <?php if (!FM_READONLY): ?>
                            <a title="<?php echo fm_t('Delete', $lang) ?>"
                               href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>"
                               onclick="return confirm('<?php echo fm_t('Delete file?') ?>');"><i class="fa fa-trash-o"
                                                                                                  style="background:#FF4136"></i></a>
                            <a title="<?php echo fm_t('Rename', $lang) ?>" href="#"
                               onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;"><i
                                        class="fa fa-pencil-square-o" style="background:#FF851B"></i></a>
                            <a title="<?php echo fm_t('Copy to...') ?>"
                               href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f,
                                   '/')) ?>"><i class="fa fa-files-o"></i></a>
                        <?php endif; ?>
                        <a title="<?php echo fm_t('Direct link', $lang) ?>"
                           href="<?php echo FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f ?>"
                           target="_blank"><i class="fa fa-link" style="background:#001f3f"></i></a>
                        <a title="<?php echo fm_t('Download', $lang) ?>"
                           href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($f) ?>"><i
                                    class="fa fa-download" style="background:#2ECC40"></i></a>
                    </td>
                </tr>
                <?php
                flush();
            ...
            if (empty($folders) && empty($files)) {
                ...
                <tr><?php if (!FM_READONLY): ?>
                        <td></td><?php endif; ?>
                    <td colspan="<?php echo !FM_IS_WIN ? '6' : '4' ?>"><em><?php echo fm_t('Folder is empty',
                                $lang) ?></em></td>
                </tr>
                <?php
            } else {
                ...
                <tr><?php if (!FM_READONLY): ?>
                    ...
                    <td class="gray" colspan="<?php echo !FM_IS_WIN ? '6' : '4' ?>">
                        <?php echo fm_t('Full size:', $lang) ?> <span title="<?php printf(fm_t('%s bytes'),
                            $all_files_size) ?>"><?php echo fm_get_filesize($all_files_size) ?></span>,
        ...
        <?php if (!FM_READONLY): ?>
            <p class="path footer-links"><a href="#/select-all" class="group-btn"
                                            onclick="select_all();return false;"><i
                            class="fa fa-check-square"></i> <?php echo fm_t('Select all', $lang) ?></a> &nbsp;
                <a href="#/unselect-all" class="group-btn" onclick="unselect_all();return false;"><i
                            class="fa fa-window-close"></i> <?php echo fm_t('Unselect all', $lang) ?></a> &nbsp;
                <a href="#/invert-all" class="group-btn" onclick="invert_all();return false;"><i
                            class="fa fa-th-list"></i> <?php echo fm_t('Invert selection', $lang) ?></a> &nbsp;
                <input type="submit" class="hidden" name="delete" id="a-delete"
                       value="<?php echo fm_t('Delete', $lang) ?>"
                       onclick="return confirm('<?php echo fm_t('Delete selected files and folders?') ?>')">
                <a href="javascript:document.getElementById('a-delete').click();" class="group-btn"><i
                            class="fa fa-trash"></i> <?php echo fm_t('Delete', $lang) ?> </a> &nbsp;
                <input type="submit" class="hidden" name="zip" id="a-zip" value="<?php echo fm_t('Zip', $lang) ?>"
                       onclick="return confirm('<?php echo fm_t('Create archive?') ?>')">
                <a href="javascript:document.getElementById('a-zip').click();" class="group-btn"><i
                            class="fa fa-file-archive-o"></i> <?php echo fm_t('Zip', $lang) ?> </a> &nbsp;
                <input type="submit" class="hidden" name="copy" id="a-copy" value="<?php echo fm_t('Copy', $lang) ?>">
                <a href="javascript:document.getElementById('a-copy').click();" class="group-btn"><i
                            class="fa fa-files-o"></i> <?php echo fm_t('Copy', $lang) ?> </a>
            </p>
        <?php endif; ?>
    </form>
...
function send_mail($path, $filename, $mailto, $message)
{
    ...
    // header
    $header = "From: File Manager Script <filemanager@mail.com>\r\n";
...
function fm_show_nav_path($path)
{
    ...
    <div class="path main-nav">
        ...
        <div class="float-right">
            <?php if (!FM_READONLY): ?>
                <a title="<?php echo fm_t('Search', $lang) ?>"
                   href="javascript:showSearch('<?php echo urlencode(FM_PATH) ?>')"><i class="fa fa-search"></i></a>
                <a title="<?php echo fm_t('Upload files', $lang) ?>"
                   href="?p=<?php echo urlencode(FM_PATH) ?>&amp;upload"><i class="fa fa-cloud-upload"
                                                                            aria-hidden="true"></i></a>
                <a title="<?php echo fm_t('New folder', $lang) ?>" href="#createNewItem"><i
                            class="fa fa-plus-square"></i></a>
            <?php endif; ?>
            <?php if (FM_USE_AUTH): ?><a title="<?php echo fm_t('Logout', $lang) ?>" href="?logout=1"><i
                        class="fa fa-sign-out" aria-hidden="true"></i></a><?php endif; ?>
        </div>
    </div>
    <?php
}
...
function fm_show_header()
{
    ...
    <head>
        ...
        <style>
            ...
            .table td label input[type="checkbox"], .table th label input[type="checkbox"] {
                margin-top: 4px;
            }
            ...
            .edit-file-actions > button, .edit-file-actions > a {
                background: #f8f8fb;
                padding: 5px 15px;
                border: 0;
                cursor: pointer;
    ...
    <div id="createNewItem" class="modalDialog">
        <div class="model-wrapper"><a href="#close" title="<?php echo fm_t('Close', $lang) ?>" class="close">X</a>
            <h2><?php echo fm_t('Create New Item', $lang) ?></h2>
            <p>
                <label for="newfile"><?php echo fm_t('Item Type', $lang) ?> &nbsp; : </label><input type="radio"
                                                                                                    name="newfile"
                                                                                                    id="newfile"
                                                                                                    value="file"><?php echo fm_t('File',
                    $lang) ?> <input type="radio" name="newfile" value="folder" checked> <?php echo fm_t('Folder',
                    $lang) ?><br><label for="newfilename"><?php echo fm_t('Item Name', $lang) ?> : </label><input
                        type="text" name="newfilename" id="newfilename" value=""><br><input type="submit" name="submit"
                                                                                            class="group-btn"
                                                                                            value="<?php echo fm_t('Create Now',
                                                                                                $lang) ?>"
    ...
    <div id="searchResult" class="modalDialog">
        <div class="model-wrapper"><a href="#close" title="<?php echo fm_t('Close', $lang) ?>" class="close">X</a>
            <input type="search" name="search" value=""
                   placeholder="<?php echo fm_t('Find a item in current folder...', $lang) ?>">
...
function fm_show_footer()
{
    ...
    <script>
      function newfolder(p) {
        ...
        var t = document.querySelector('input[name="newfile"]:checked').value;
        if (n !== null && n !== '' && t) {
          window.location.hash = '#';
          window.location.search = 'p=' + encodeURIComponent(p) + '&new=' + encodeURIComponent(n) + '&type=' +
              encodeURIComponent(t);
        }
      }

      function rename(p, f) {
        var n = prompt('<?php global $lang; echo fm_t('New name', $lang) ?>', f);
        if (n !== null && n !== '' && n != f) {
          window.location.search = 'p=' + encodeURIComponent(p) + '&ren=' + encodeURIComponent(f) + '&to=' +
        ...
        timeout = setTimeout(function() {
          if (searchTerms.length >= 2) {
            ...
            res.folders.forEach(function(d) {
              f1 += '<li class="' + d.type + '"><a href="?p=' + d.path + '">' + d.name + '</a></li>';
            });
            res.files.forEach(function(d) {
              f2 += '<li class="' + d.type + '"><a href="?p=' + d.path + '&view=' + d.name + '">' + d.name +
                  '</a></li>';
            });
            document.getElementById('searchresultWrapper').innerHTML = '<div class="model-wrapper">' + f1 + f2 +
                '</div>';
          }
        }, 500);
      };
      ...
      function backup(p, f) {
        var http = new XMLHttpRequest();
        var params = 'path=' + p + '&file=' + f + '&type=backup&ajax=true';
        http.open('POST', '', true);
        http.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
        http.onreadystatechange = function() {
          if (http.readyState == 4 && http.status == 200) {
      ...
      function edit_save(i, atr) {
        var contents = (atr == 'ace') ? editor.getSession().getValue() : document.getElementById('normal-editor').value;
        if (!!contents) {
